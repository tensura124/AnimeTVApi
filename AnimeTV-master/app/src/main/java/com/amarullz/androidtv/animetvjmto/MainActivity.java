...
//        initLogcat(); // Uncomment if needed
    }
    ...
    void showRewardedAdBeforeEpisode() {
        if (rewardedAd != null) {
            rewardedAd.show(this, rewardItem -> {
                // Play episode after watching the ad
                playEpisode();
            });
        } else {
            loadRewardedAd(); // Reload the ad if not ready
        }
    }
    ...
    DeepSeekApiService apiService = ApiClient.getRetrofit().create(DeepSeekApiService.class);

    void searchManga(String query) {
        apiService.searchManga(query).enqueue(new retrofit2.Callback<SearchResponse>() {
            @Override
